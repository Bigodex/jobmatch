...
// -------------------------------------------------------
// Agora no mesmo padrão do StepExperience:
...
class _EditExperienceScreenState
  ...
  void initState() {
    ...
    if (widget.experiences.isNotEmpty) {
      ...
      descriptions = widget.experiences
          .map(
            (e) => TextEditingController(
  ...
  void _recalculateState() {
    _hasChanged = _hasExperiencesChanged();
  ...
  bool _hasExperiencesChanged() {
    ...
    for (int i = 0; i < current.length; i++) {
      ...
      if (currentItem['company'] != originalItem['company']) return true;
      if (currentItem['role'] != originalItem['role']) return true;
  ...
  List<Map<String, dynamic>> _buildComparableSnapshot() {
    ...
    for (int index = 0; index < companies.length; index++) {
      if (!_isExperienceTouched(index)) continue;
      ...
      items.add({
        ...
        'end': isCurrentJobs[index] ? '' : endDateControllers[index].text.trim(),
        'logo': (logoImages[index] ?? '').trim(),
        'isCurrent': isCurrentJobs[index],
      });
    }
  ...
  List<Map<String, dynamic>> _buildOriginalComparableSnapshot() {
    ...
    for (final experience in widget.experiences) {
      final originalIsCurrent = experience.endDate == null;

      items.add({
        'company': experience.company.trim(),
        'role': experience.role.trim(),
        'description': experience.description.trim(),
        'start': _isStorageEmptyDate(experience.startDate)
            ? ''
            : _formatMonthYear(experience.startDate),
        'end': originalIsCurrent || experience.endDate == null
            ? ''
            : _formatMonthYear(experience.endDate!),
        'logo': (experience.logoUrl ?? '').trim(),
  ...
  Future<void> _save() async {
    ...
    if (touchedIndexes.isEmpty) {
      setState(() {
        _errorMessage = 'Preencha pelo menos uma experiência.';
      });
      return;
    }
    ...
    for (final index in touchedIndexes) {
      ...
      if (company.isEmpty) {
        setState(() {
          _errorMessage = 'Preencha a empresa ou remova o item vazio.';
        });
        return;
      }
      ...
      if (company.length < 2) {
        setState(() {
          _errorMessage = 'O nome da empresa está curto demais.';
        });
        return;
      }
      ...
      if (role.isEmpty) {
        setState(() {
          _errorMessage = 'Preencha o cargo da experiência "$company".';
        });
        return;
      }
      ...
      if (role.length < 2) {
        setState(() {
          _errorMessage = 'O cargo da experiência "$company" está curto demais.';
        });
        return;
      }
      ...
      if (startDateText.isEmpty || startDate == null) {
        setState(() {
          _errorMessage = 'Digite a data de início em MM/AAAA para "$company".';
        });
        return;
      }
      ...
      if (description.isEmpty) {
        setState(() {
          _errorMessage = 'Descreva sua atuação em "$company".';
        });
        return;
      }
      ...
      if (description.length < 10) {
        setState(() {
          _errorMessage = 'A descrição de "$company" está curta demais.';
        });
        return;
      }
    }
